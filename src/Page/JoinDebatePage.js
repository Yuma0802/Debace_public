  ...
  useEffect(() => {(async() => {
    if(userT){
      try{
        let preElementList = []
        let preCount = 0
        const collectionRef = collection(db,'users',userId,'join')
        const debateQuery = query(collectionRef,orderBy('joinAt','desc'),limit(10))
    ...
    if(nextFlag === true){
      if(dCount === 10){
        ...
        try{
          ...
          let preElementList = []
          const collectionRef = collection(db,'users',userId,'join')
          const nextQuery = query(collectionRef,orderBy('joinAt','desc'),startAfter(lastRef),limit(10))
  ...
  return (
    <>
      ...
      <SSelectBoxWrap>
        ...
        <SFavoriteDebateTitle>参加した議論</SFavoriteDebateTitle>
