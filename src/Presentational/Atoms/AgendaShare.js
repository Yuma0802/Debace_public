...
const AgendaShare = (props) => {

  const title = `「${props.title}」　あなたもDebaceで議論をしよう！`;

  const [isPopoverOpen, setIsPopoverOpen] = useState(false);
  const currentUrl = window.location.href; 
  return (
    <SShareBtm>
      <SShareBtmIcon onClick={() => setIsPopoverOpen(!isPopoverOpen)}>
...
const SShareBtm = styled.div`
  position: relative; 
  margin-bottom: 10px;
`;

const SShareBtmIcon = styled.div`
  cursor: pointer; 
  &:hover::after {
